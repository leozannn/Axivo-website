    ...
    >
      <div className="max-w-7xl mx-auto">
        <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true }}>
          ...
          <motion.p variants={fadeUp} className="text-base mb-8 max-w-xl" style={{ color: '#64748B', lineHeight: 1.7 }}>
            {t(tr.subtitle, lang)}
          </motion.p>

          {/* Horizontal drawing line — desktop only */}
          <div className="hidden md:block relative h-px mb-4 mx-1">
            <div className="absolute inset-0" style={{ background: 'rgba(0,0,0,0.08)' }} />
            <motion.div
              className="absolute top-0 left-0 h-full"
              style={{ background: 'linear-gradient(90deg, #00C8FF, rgba(0,200,255,0.4))', transformOrigin: 'left' }}
              initial={{ scaleX: 0 }}
            ...
            <div className="hidden md:grid md:grid-cols-5 gap-4">
              {tr.steps.map((step, i) => (
                <motion.button
                  ...
                  transition={{ duration: 0.5, delay: 0.2 + i * 0.22, ease: EASE }}
                  viewport={{ once: true }}
                  whileHover={{ y: -4, boxShadow: activeStep === i ? undefined : '0 4px 24px rgba(0,200,255,0.15)' }}
                  ...
                  style={{
                    ...
                    boxShadow: activeStep === i ? '0 4px 24px rgba(0,200,255,0.25), 0 0 12px rgba(0,200,255,0.15)' : 'none',
                  }}
                >
                  <div
                    className="text-2xl font-bold mb-2"
                    style={{ fontFamily: 'Space Grotesk, sans-serif', color: activeStep === i ? '#00C8FF' : '#CBD5E1' }}
                  >
                    {step.num}
                  </div>
                  <div
                    className="font-semibold text-sm mb-3"
                    style={{ fontFamily: 'Space Grotesk, sans-serif', color: activeStep === i ? '#fff' : '#1A1A2E' }}
                  >
                    {t(step.title, lang)}
                  </div>
                  <motion.div
            ...
            <div className="flex flex-col gap-3 md:hidden">
              {tr.steps.map((step, i) => (
                <div key={step.num}>
                  <button
                    onClick={() => handleClick(i)}
                    className="w-full text-left rounded-xl transition-colors duration-200"
                    style={{
                      background: activeStep === i ? '#1A1A2E' : '#fff',
                      border: activeStep === i ? '2px solid #00C8FF' : '2px solid #E2E8F0',
                      padding: '14px 20px',
                    ...
                    {activeStep === i && (
                      ...
                      >
                        <div
                          ...
                          style={{ background: '#1A1A2E', border: '1px solid rgba(0,200,255,0.3)', borderTop: 'none' }}
                        >
